...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        x = x.view(B, H, W, -1) #感觉不能粗暴的把HW去掉，不然滑窗没法做
        # x = x.view(B, H, W, C) #感觉不能粗暴的把HW去掉，不然滑窗没法做
...
class SwinTransformerSys(nn.Module): #他就是改了这里
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, final_upsample="expand_first", **kwargs): #final_upsample多的
        super().__init__()
        ...
        self.num_features_up = int(embed_dim * 2) #多的
        self.mlp_ratio = mlp_ratio
        ...
        # build decoder layers  #这里的模块都是多的
