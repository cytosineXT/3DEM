...
def generate_rcs_curve(device, plane1, weight, f, save_path, batch_size=1):
    ...
    # trainGTdir = '/home/ljm/workspace/datasets/mulbb7c_mie_pretrain'
    # valGTdir = '/home/ljm/workspace/datasets/mulbb7c_mie_val'
    # trainGTdir = '/home/ljm/workspace/datasets/mulbb7c_mie_rcs360'
    # valGTdir = '/home/ljm/workspace/datasets/mulbb7c_mie_rcs360val'
    trainGTdir = '/mnt/truenas_jiangxiaotian/allplanes/mie/b7fd_mie_train'
    valGTdir = '/mnt/truenas_jiangxiaotian/allplanes/mie/b7fd_mie_val'
    ...
    # 推理 RCS 值
    rcs_data = []
    for phi in tqdm(phi_values, desc=f"推理{f}GHz RCS值", ncols=70):
        # 构造输入
        phi = int(phi)
        freq = float(f)
    ...
    ax.plot(alpha_values, full_rcs_data_db, label=f'Inference Output', color='skyblue', zorder=1)
    ...
    # GTtrain_phi=[0, 150]
    # GTtrain_phi=[0, 30, 150, 210]
    # GTtrain_phi=[0, 30, 60, 90, 120, 150, 210, 240, 270, 300, 330]
    # GTtrain_phi=[0, 30, 60, 90, 120, 150, 210, 240, 270, 300]
    # GTtrain_values=[0.275, 0.2694, 0.2694, 0.24, 0.2694, 0.26, 0.26, 0.26, 0.26, 0.22, 0.26]
    # GTtrain_values_db = [-4.96, -5.3]
    # GTtrain_values_db = [-4.96, -4.81, -5.3, -5.3]
    # GTtrain_values_db = [-4.96, -4.81, -5.46, -7.25, -5.85, -5.3, -5.3, -5.85, -7.25, -5.46, -4.81]
    # GTtrain_values_db = [-4.96, -4.81, -5.46, -7.25, -5.85, -5.3, -5.3, -5.85, -7.25, -5.46]

    print(f'train{GTtrain_phi}')
    # print(GTtrain_values)
    print(GTtrain_values_db)
    GTtrain_alpha = np.deg2rad(np.array(GTtrain_phi))
    ax.scatter(GTtrain_alpha, GTtrain_values_db, label='Train Groundtruth', color='green', s=10, zorder=2)
    ...
    # GTval_phi = [180, 330]
    # GTval_values = [0.270,]
    # GTval_values_db = [-5.592]
    # GTval_values_db = [-5.592, -4.81]
    print(f'val{GTval_phi}')
    # print(GTval_values)
    print(GTval_values_db)
    GTval_alpha = np.deg2rad(np.array(GTval_phi))
    ax.scatter(GTval_alpha, GTval_values_db, label='Val Groundtruth', color='red', s=10, zorder=2)
...
if __name__ == "__main__":
    ...
    # weight = "output/train/1222bb7cpretrain_m1753.pt"  # 网络权重路径
    # weight = "/mnt/SrvUserDisk/JiangXiaotian/workspace/3DEM/output/newtrain/0112_bb7c_90RCS360_allCondL1_e600lr0.1_sd7smFalse_cuda:1_2m0154/last.pt"  # 网络权重路径
    weight = "/home/jiangxiaotian/workspace/3DEM/outputGNN/b7fd_50fine_m0090.pt"  # 网络权重路径
    lsp = ['b7fd']
    # lsf = [0.165]  # 指定频率列表 (GHz)
    lsf = [0.11,0.12,0.13,0.14,0.16,0.17]  # 指定频率列表 (GHz)
    # lsf = np.arange(0.1, 0.21, 0.005)
    for plane1 in lsp:
        for f in lsf:
            f = float(round(f, 3))
            dirname = f'./outputGNN/RCS/rcs{date}/try1'
            os.makedirs(dirname, exist_ok=True)

            filename = f"{plane1}_50finem0090_f{f:.3f}.png"
