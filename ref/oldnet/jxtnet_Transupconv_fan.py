...
class MeshEncoderDecoder(Module):
    ...
    def __init__(
        ...
        # self.fc1d1 = nn.Sequential(
        #         # nn.Linear(int(self.paddingsize/(2**encoder_layer)), int(self.paddingsize/(2**encoder_layer))),
        #         FANLayer(input_dim=int(self.paddingsize/(2**encoder_layer)), output_dim=int(self.paddingsize/(2**encoder_layer)), with_gate=True),
        #         nn.SiLU(),
        #         # nn.Linear(int(self.paddingsize/(2**encoder_layer)), decoder_outdim*8*45*90),#388800
        #         FANLayer(input_dim=int(self.paddingsize/(2**encoder_layer)), output_dim=decoder_outdim*8*45*90, with_gate=True),
        #         nn.LayerNorm(decoder_outdim*8*45*90)).to(device)
        # self.fc1d1 = nn.Sequential(
        #         # nn.Linear(int(self.paddingsize/(2**encoder_layer)), int(self.paddingsize/(2**encoder_layer))),
        #         FANLayer(input_dim=int(self.paddingsize/(2**encoder_layer)), output_dim=int(self.paddingsize/(2**encoder_layer)), with_gate=False),
        #         nn.SiLU(),
        #         # nn.Linear(int(self.paddingsize/(2**encoder_layer)), decoder_outdim*8*45*90),#388800
        #         FANLayer(input_dim=int(self.paddingsize/(2**encoder_layer)), output_dim=decoder_outdim*8*45*90, with_gate=False),
        #         nn.LayerNorm(decoder_outdim*8*45*90)).to(device)
        # if exp_setting == 0:
