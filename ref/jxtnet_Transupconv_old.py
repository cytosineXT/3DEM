...
#--------------------------------------------------------face预处理 得到特征--------------------------------------------------------------------------
    ...
    def decode( #decoder输入：torchsize(1,33564,576) 长度33564待定，维度576固定
        ...
        # condfreq1 = self.incident_freq_linear1(lg_emfreq.unsqueeze(1).float())  #缩放因子，加强频率的影响，因为现在看来频率没啥影响，网络还没学到根据频率而变..不大行 发现是应该要归一化
        # condfreq2 = self.incident_freq_linear2(lg_emfreq.unsqueeze(1).float())
