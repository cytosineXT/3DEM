...
@torch.no_grad()
def get_derived_face_features( #这一段代码好像有点问题，重点关注他的数据结构，而不是内容。回头要重写代码，反正还要加进去新的feature
    ...
    angles  = derive_angle(face_coords, shifted_face_coords) #但是这里给他的踏马不是顶点坐标么，真奇怪了妈的，做三角形的内角怎么是用顶点做呢，应该是用边做吧，草拟吗
    edge1, edge2, *_ = (face_coords - shifted_face_coords).unbind(dim = 2) #这里是坐标相减得到边，这就很合理。所以上面的夹角不应该tm用边矢量的夹角来做么，草，逆天啊
    normals = l2norm(torch.cross(edge1, edge2, dim = -1)) #然后也用边叉乘得到法向量，很合理。
    area = normals.norm(dim = -1, keepdim = True) * 0.5 #虽然两条边叉乘/2得到三角形面积，但是这里又不合理了，已经归一化了 area全是0.5.。。要提前做。
