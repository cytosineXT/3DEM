...
def get_derived_face_featuresjxt(
    ...
    incident_freq_mtx = in_em[2].repeat(area.shape) #得到入射波频率的矩阵torch.Size([1, 33564, 1])
...
#----------------------------------------------------------------------------------------------------------------------------------
    ...
    def decode(
        ...
        for resnet_block in self.decoders: #jxt 2024年4月3日23:58:05 现在输入改完了，输出还没改，还要加一个入射角和频率的值。
            x = resnet_block(x, mask = conv_face_mask) #128 192 256 384
            '''decoder_dims_through_depth: Tuple[int, ...] = (
            128, 128, 128, 128,
            192, 192, 192, 192,
            256, 256, 256, 256, 256, 256,
            384, 384, 384  ),    ''' #这里，先把特征从576维映射到128维，然后把128卷成192 256 384，提出一堆特征啊
...
#------------------------------------------------------------------进Decoder---------------------------------------------------------------------------------------------
        ...
        # '''torch.Size([1, 33564, 9, 128])'''
        # # 在这儿动手脚！这是把128维离散代码undiscretize连续回一个连续值，得到9。那我就是把9后面加10theta 11phi 12freq，特征也是128 然后undiscretize回三个数字。

        pred_out = self.to_coor_logits_emincident(decode) #jxt
        '''torch.Size([1, 33564, 12, 128])'''
        # 在这儿动手脚！这是把128维离散代码undiscretize连续回一个连续值，得到9。那我就是把9后面加10theta 11phi 12freq，特征也是128 然后undiscretize回三个数字。

        in_em_mtx_inloss = torch.tensor(in_em).repeat(face_coordinates[:,:,0].unsqueeze(-1).shape) #jxt
        '''torch.Size([1, 33564, 3])'''
        pred_face_coords = pred_out[:, :, :9, :]
        pred_em = pred_out[:, :, 9:, :] #jxt
        # pred_thetaa = pred_out[:, :, 9, :]
        # pred_phii = pred_out[:, :, 10, :]
        # pred_freq = pred_out[:, :, 11, :]
        '''三个torch.Size([1, 33564, 128])'''
        '''一个torch.Size([1, 33564, 3, 128])'''
        ...
        with autocast(enabled = False):
            ...
            target_one_hot = torch.zeros_like(pred_log_prob).scatter(1, face_coordinates, 1.) #但是是用原始坐标值+预测坐标形状 做的一个成了0-1 onehot值矩阵 torch.mean(target_one_hot)=tensor(0.0078)；torch.max(target_one_hot)=tensor(1.0)
            ''''torch.Size([1, 128, 302076])'''

            if self.bin_smooth_blur_sigma >= 0.:
                target_one_hot = gaussian_blur_1d(target_one_hot, sigma = self.bin_smooth_blur_sigma) #大小还是没变torch.Size([1, 128, 302076])，但是成了0-1 onehot值 torch.mean(target_one_hot) = tensor(0.0078)；torch.max(target_one_hot)=tensor(0.9192) 好像是做了一个不改变均值的缩放，把最大值拉低了.草但是最小值还是0，这是怎么做到的。。 这是一个一维高斯模糊，让值平滑，不改变分布的均值。
                '''torch.Size([1, 128, 302076])'''
        ...
        #----------------------------------------------------------------------------------------------
        #-------------------------------------------emin-loss--------------------------------------------------- jxtttttttt
        # em_recon_losses = 0
        pred_em = rearrange(pred_em, 'b ... c -> b c (...)') #torch.Size([1, 33564, 9, 128])→torch.Size([1, 128, 302076])
        in_em_mtx_inloss = rearrange(in_em_mtx_inloss, 'b ... -> b 1 (...)') #face_coordinates从torch.Size([1, 33564, 9])→torch.Size([1, 1, 302076])
        pred_log_probem = pred_em.log_softmax(dim = 1) #矩阵大小torch.Size([1, 128, 302076])没变，矩阵元素值经过了log_softmax
        '''torch.Size([1, 128, 302076])'''
        target_one_hotem = torch.zeros_like(pred_log_probem).scatter(1, in_em_mtx_inloss, 1.)
        ''''torch.Size([1, 128, 302076])'''
        if self.bin_smooth_blur_sigma >= 0.:
            target_one_hotem = gaussian_blur_1d(target_one_hotem, sigma = self.bin_smooth_blur_sigma)
            '''torch.Size([1, 128, 302076])'''
        em_recon_losses = (-target_one_hotem * pred_log_probem).sum(dim = 1)
        em_recon_loss = em_recon_losses.mean()


        # calculate total loss
        #recon_loss = tensor(4.9608, grad_fn=<MeanBackward0>); commit_loss = tensor([0.8444, 0.0765], grad_fn=<StackBackward0>)
        total_loss = recon_loss + em_recon_loss + commit_loss.sum() * self.commit_loss_weight #jxt
        ...
        loss_breakdown = (recon_loss, em_recon_loss, commit_loss)
