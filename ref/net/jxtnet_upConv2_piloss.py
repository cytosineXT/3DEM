...
def get_derived_face_featuresjxt(
    ...
    incident_freq_mtx = in_em[0][2].repeat(area.shape) #得到入射波频率的矩阵torch.Size([1, 33564, 1]) 感觉取对数不是那个意思，对数坐标只是看起来的，不是实际上的？
...
class MeshAutoencoder(Module):
    ...
    def __init__(
        ...
        super().__init__()
        #----------------------------------------------------jxt decoder----------------------------------------------------------
        self.conv1 = nn.ConvTranspose1d(784, 128, 5, stride=2, padding=1) #输入为(batch,dim,length)
        ...
        self.bn1d2 = nn.BatchNorm1d(64)  # Batch Normalization after the second convolutional layer
        ...
        self.bn1d3 = nn.BatchNorm1d(32)  # Batch Normalization after the third convolutional layer
        ...
        self.conv6 = nn.ConvTranspose1d(8, 2, 5, stride=2, padding=1)
        self.pool6 = nn.MaxPool1d(2)  # Add a max pooling layer after the fifth convolutional layer
        #----------------------------------------------------jxt decoder----------------------------------------------------------

        # # Decoder2
        # self.fc1 = nn.Linear(in_features=21000*8, out_features=1024)
        # self.fc2 = nn.Linear(in_features=1024, out_features=361*720)

        # Decoder3
        self.upconv1 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.bn1 = nn.BatchNorm2d(128)  # 添加的批量归一化层1
        self.conv1_1 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)  # 添加的卷积层1
        self.conv1_2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)  # 添加的卷积层2
        self.bn1_1 = nn.BatchNorm2d(128)  # 添加的批量归一化层1
        self.bn1_2 = nn.BatchNorm2d(128)  # 添加的批量归一化层2
        self.upconv2 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.bn2 = nn.BatchNorm2d(64)  # 添加的批量归一化层1
        self.conv2_1 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)  # 添加的卷积层1
        self.conv2_2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)  # 添加的卷积层2
        self.bn2_1 = nn.BatchNorm2d(64)  # 添加的批量归一化层1
        self.bn2_2 = nn.BatchNorm2d(64)  # 添加的批量归一化层2
        self.upconv3 = nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2, output_padding=1)
        self.bn3 = nn.BatchNorm2d(32)
        self.conv3_1 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)  # 添加的卷积层1
        self.conv3_2 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)  # 添加的卷积层1
        self.bn3_1 = nn.BatchNorm2d(32)  # 添加的批量归一化层1
        self.bn3_2 = nn.BatchNorm2d(32)  # 添加的批量归一化层2
        self.conv1x1 = nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0)   #1×1卷积，把多的维度融合了
        ...
        self.discretize_emfreq = partial(discretize, num_discrete = num_discrete_emfreq, continuous_range = (0.001,8.0)) #2024年5月11日15:28:15我草 是不是没必要离散，这个情况，是不是其实我的freq本身其实就已经离散的了，不用我再人为离散化一次？只是embedding的时候他映射到embedding空间之后，隐含的空间关系就能实现我“连续回归”的目的？而且280个点离散到128个离散值，本身就有问题吧你妈的
...
#----------------------------------------------------------------------------------------------------------------------------------
        ...
        _, _, num_vertices_per_face = faces.shape #不关心faces的batch，取出后两个元素
        # _, num_faces, num_vertices_per_face = faces.shape #不关心faces的batch，取出后两个元素
        # print(f'Encoder Step0 可删用时：{(time.time()-ticc):.4f}s')
        # ticc = time.time()
        
        assert self.num_vertices_per_face == num_vertices_per_face #断言 检查预设每面顶点数量是否与输入数据的格式相等，不相等的话抛出异常

        face_without_pad = faces#.masked_fill(~rearrange(face_mask, 'b nf -> b nf 1'), 0) #这应该是把face用face mask清洗了一遍.      
        # continuous face coords
        # print(f'Encoder Step00 可删用时：{(time.time()-ticc):.4f}s')
        # ticc = time.time()

        # face_coords = get_at('b [nv] c, b nf mv -> b nf mv c', vertices, face_without_pad) #用清洗过的索引face和点坐标得到有坐标的face 输入torch.Size([2, 10400, 3]) torch.Size([2, 20804, 3])
        #输出torch.Size([2, 20804, 3, 3])  face_coords[0][0][0]=tensor([-0.4463, -0.0323, -0.0037], device='cuda:0')
        face_coords = jxtget_face_coords(vertices, face_without_pad) 
...
#----------------------------------------------------------------------------------------------------------------------------------
    ...
    def decode( #decoder输入：torchsize(1,33564,576) 长度33564待定，维度576固定
        ...
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        pad_size = 32400 - x.size(1)
        x = F.pad(x, (0, 0, 0, pad_size)) #x.shape=torch.Size([2, 20804, 576])
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])

        x = x.view(x.size(0), -1, 32400)  # adjust to match the input shape, 1,576,33564
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        
        # ------------------------1D upConv------------------------------
        x = self.conv1(x)
        x = self.pool1(x)  # torchsize(1,128,31680)
        x = self.bn1d1(x)
        x = F.relu(x)
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        x = self.conv2(x)
        x = self.pool2(x)  # torchsize(1,64,31680)
        x = self.bn1d2(x)
        x = F.relu(x)
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        x = self.conv3(x)
        x = self.pool3(x)  # torchsize(1,32,31680)
        x = self.bn1d3(x)
        x = F.relu(x)
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        ...
        # x = self.conv4(x)
        # x = self.pool4(x)  # torchsize(1,16,31680)
        # # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        # x = self.conv5(x)
        # x = self.pool5(x)  # torchsize(1,8,31680)=168000
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        # x = self.conv6(x) #少卷一层，留8个特征维度
        # x = self.pool6(x)  # torchsize(1,2,31680)
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        

        # x=x.view(16,45,-1)     # torchsize(1,64800) #64800=16*45*90
        # # print(x.size())
        # print(x.shape,x.shape[0]*x.shape[1]*x.shape[2])

        # x = torch.randn(1, 1, 64800)
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        ...
        # x=F.relu(self.fc1(x)) # torchsize(1,1024)
        # # print(x.size())
        # x=F.relu(self.fc2(x)) # torchsize(1,259920)
        # # print(x.size())

        # x=x.view(1,361,-1) # torchsize(1,361,720)
        # print(x.size())
