...
def jxtget_face_coords(vertices, face_indices):
    ...
    """
    #face_indices.shape=torch.Size([2, 20804, 3]),vertices.shape = torch.Size([2, 10400, 3])
    batch_size, num_faces, num_vertices_per_face = face_indices.shape
    num_coordinates = vertices.shape[-1]

    # 做一次reshape，将face_indices变为1D张量，然后用它来索引点坐标张量
    reshaped_face_indices = face_indices.reshape(batch_size, -1)  # 形状为 (b, nf*c)

    # 使用索引张量获取具有坐标的面
    face_coords = torch.gather(vertices, 1, reshaped_face_indices.unsqueeze(-1).expand(-1, -1, num_coordinates))

    # 还原形状
    face_coords = face_coords.reshape(batch_size, num_faces, num_vertices_per_face, num_coordinates)
...
class MeshAutoencoder(Module):
    ...
    def __init__(
        ...
        self.conv1_1 = nn.Conv2d(8, 8, kernel_size=3, stride=1, padding=1)  # 添加的卷积层1
        self.conv1_2 = nn.Conv2d(8, 8, kernel_size=3, stride=1, padding=1)  # 添加的卷积层2
        self.bn1_1 = nn.BatchNorm2d(8)  # 添加的批量归一化层1
        self.bn1_2 = nn.BatchNorm2d(8)  # 添加的批量归一化层2
        self.upconv2 = nn.ConvTranspose2d(8, 4, kernel_size=2, stride=2)
        self.conv2_1 = nn.Conv2d(4, 4, kernel_size=3, stride=1, padding=1)  # 添加的卷积层1
        self.conv2_2 = nn.Conv2d(4, 4, kernel_size=3, stride=1, padding=1)  # 添加的卷积层2
        self.bn2_1 = nn.BatchNorm2d(4)  # 添加的批量归一化层1
        self.bn2_2 = nn.BatchNorm2d(4)  # 添加的批量归一化层2
        self.upconv3 = nn.ConvTranspose2d(4, 1, kernel_size=2, stride=2, output_padding=1)
...
#--------------------------------------------------------face预处理 得到特征--------------------------------------------------------------------------
        ...
        # print(f'Encoder Step3用时：{(time.time()-ticc):.4f}s')
        # ticc = time.time()
        ...
        face_edges = face_edges[face_edges_mask] #torch.Size([166432, 2])
        face_edges = rearrange(face_edges, 'be ij -> ij be') #torch.Size([2, 166432])
...
#----------------------------------------------------------------------------------------------------------------------------------
        ...
        # print(f'Encoder用时：{(time.time()-timeen):.4f}s')
    ...
    def decode( #decoder输入：torchsize(1,33564,576) 长度33564待定，维度576固定
        ...
        x = F.pad(x, (0, 0, 0, pad_size)) #现在是torchsize(1,31680,576)
        ...
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
        x = self.conv3(x)
        x = self.pool3(x)  # torchsize(1,32,316800)
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2])
