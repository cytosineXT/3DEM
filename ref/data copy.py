...
# # 直接使用 torch.Tensor 替代
Vertices = torch.Tensor  # 3 coordinates
Faces = torch.Tensor      # 3 vertices
...
def derive_face_edges_from_faces(
    faces: torch.Tensor,  # 形状应为 [b, nf, 3]
    pad_id=-1,
    neighbor_if_share_one_vertex=False,
    include_self=True
) -> torch.Tensor:  # 返回形状应为 [b, e, 2]
    '''
    faces.shape = torch.Size([1, 33564, 3])
    faces = tensor([[[    0,     2,     1],
         [    4,     3,     1],
         [    4,     1,     2],
         ...,
    ...
    for face, face_edge_mask in zip(faces, face_edges_masks):
        ...
        '''
        shared_vertices = tensor([[[[ True, False, False],
          [False,  True, False],
          [False, False,  True]],
        ...
        '''
        num_shared_vertices = tensor([[3, 1, 2,  ..., 0, 0, 0],
        [1, 3, 2,  ..., 0, 0, 0],
        [2, 2, 3,  ..., 0, 0, 0],
        ...,
        [0, 0, 0,  ..., 3, 0, 0],
        ...
        '''
        is_neighbor_face = tensor([[ True, False,  True,  ..., False, False, False],
...
# def myderive_face_edges_from_faces(
#     faces: TensorType['b', 'nf', 3, int],
#     pad_id = -1,
#     neighbor_if_share_one_vertex = False,
#     include_self = True
...
def myderive_face_edges_from_faces(
    ...
    for b, (face, face_edge_mask) in enumerate(zip(faces, face_edges_masks)):

        face_edge = []

        for i in tqdm(range(max_num_faces)):
            shared_vertices = (face[i].unsqueeze(0) == face).sum(dim=-1)
            num_shared_vertices = shared_vertices

            is_neighbor_face = (num_shared_vertices >= face_edges_vertices_threshold) & face_edge_mask[b]

            if not include_self:
                is_neighbor_face &= num_shared_vertices != 3

            face_edge_i = all_edges[is_neighbor_face]
            face_edge.append(face_edge_i)

        face_edges.append(torch.cat(face_edge, dim=0))
