...
# rcsdir = r'/mnt/Disk/jiangxiaotian/puredatasets/b827_xiezhen_val' #T7920 
...
pretrainweight = r'./output/train/0518upconv3L1_b827_/best.pt' #T7920
loadobj = r'./planes/b82731071bd39b66e4c15ad8a2edd2e.obj'
loadpt = r'./planes/b82731071bd39b66e4c15ad8a2edd2e.pt'

save_dir = str(increment_path(Path(ROOT / "output" / "train" /'0518upconv3L1_b827_debug'), exist_ok=False))##日期-NN结构-飞机-训练数据-改动
...
for i in range(epoch):
    ...
    for in_em1,rcs1 in tqdm(dataloader,desc=f'epoch:{i+1},datasets进度,lr={scheduler.get_last_lr()[0]:.5f}',ncols=130,postfix=f'上一轮的epoch:{i},loss_mean:{(epoch_loss1/dataset.__len__()):.4f}'):
        ...
        # planesur_face = planesur_face.repeat(in_em1.shape[0], 1, 1)
        # planesur_vert = planesur_vert.repeat(in_em1.shape[0], 1, 1)
        # planesur_faceedge = planesur_faceedge.repeat(in_em1.shape[0], 1, 1)
...
#2024年5月18日11:00:14 实现了SSIM和PSNR指标 解决了多batchsize的bug，现用batchsize=8训练！ 修改了L1loss 加了mse指标。 准备实现TV全变分正则化