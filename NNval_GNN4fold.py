...
def valmain(draw, device, weight, rcsdir, save_dir, logger, epoch, batchsize, trainval=False, draw3d=False,lgrcs=False,decoder_outdim=3,encoder_layer=6,paddingsize=18000, n=4, middim=64,attnlayer=0,valdataloader=None):
    tic = time.time()
...
if __name__ == '__main__':
    ...
    weight = r'/mnt/SrvUserDisk/JiangXiaotian/Dell7920_Double3090_zx/jxtnet/output/train/old/0518upconv3L1_b827_xiezhen/best.pt'
    rcsdir = r'/mnt/Disk/jiangxiaotian/puredatasets/b827_xiezhen_val'
    
    from datetime import datetime
    date = datetime.today().strftime("%m%d")
    save_dir = str(increment_path(Path(ROOT / "output" / "inference" /f'{date}_GNNconv3L1_b827'), exist_ok=False))
