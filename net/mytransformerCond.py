...
class TransformerEncoderLayer(Module):
    ...
    Examples::
        >>> encoder_layer = nn.TransformerEncoderLayer(d_model=512, nhead=8) #输入length batch dim
        >>> src = torch.rand(10, 32, 512)
        >>> out = encoder_layer(src)

    Alternatively, when ``batch_first`` is ``True``:
        >>> encoder_layer = nn.TransformerEncoderLayer(d_model=512, nhead=8, batch_first=True) #输入batch length dim
...
class TransformerDecoderLayer(Module): #感觉可以用TransformerEncoder和Decoder，但是他的Decoder是自回归的而且是时序生成，能这么生成图片吗
    ...
    Alternatively, when ``batch_first`` is ``True``:
        >>> decoder_layer = nn.TransformerDecoderLayer(d_model=512, nhead=8, batch_first=True) #这么说32是batch，512肯定是dim，那10和20是length？
...
class PositionalEncoding(Module):
    def __init__(self, d_model, max_len=25000):
        ...
        pe = torch.zeros(max_len, d_model)# 初始化位置编码矩阵，大小为 (max_len, d_model)
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)# 生成位置索引 (0, 1, 2, ..., max_len-1)，形状为 (max_len, 1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-torch.log(torch.tensor(10000.0)) / d_model))# 计算div_term，用于对位置进行缩放，形状为 (d_model/2,)
        pe[:, 0::2] = torch.sin(position * div_term) # 将位置编码的偶数列设为 sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)# 将位置编码的奇数列设为 cos(position * div_term)
        pe = pe.unsqueeze(0).transpose(0, 1) # 增加一个批次维度，形状变为 (max_len, 1, d_model)
        self.register_buffer('pe', pe) # 将位置编码矩阵注册为buffer，这样它不会作为模型参数，但会在模型保存和加载时一起保存和加载
...
class TransformerWithPooling(Module):
    def __init__(self, d_model, nhead, dim_feedforward, num_layers, pool_size, activation='silu'):
    ...
    def pooling(self, x):
        # Assuming x shape is (seq_len, batch_size, d_model)
        seq_len, batch_size, d_model = x.shape
        new_seq_len = seq_len // self.pool_size #原来是torch.Size([22500, 1, 576])
        x = x.reshape(x.shape[-1], -1, x.shape[0])  # 要变成torch.Size([576, 1, 22500]) ( d_model, batch_size, seq_len)
