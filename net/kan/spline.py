...
def coef2curve2(x_eval, grid, coef, k, device="cpu"):
    '''
    converting B-spline coefficients to B-spline curves. Evaluate x on B-spline curves (summing up B_batch results over B-spline basis).
    ...
    '''
    # x_eval: (size, batch), grid: (size, grid), coef: (size, coef)
    # coef: (size, coef), B_batch: (size, coef, batch), sum over coef
    
    # Compute the B_batch tensor
    B = B_batch(x_eval, grid, k, device=device)
    
    # Ensure the dtype of coef matches that of x_eval
    if coef.dtype != x_eval.dtype:
        coef = coef.to(x_eval.dtype)
    
    # Initialize the result tensor
    y_eval = torch.zeros((coef.shape[0], x_eval.shape[1]), device=device)
    
    # Compute y_eval by summing the element-wise product over the second dimension (coef dimension)
    for i in range(coef.shape[0]):  # Iterate over splines
        for k in range(x_eval.shape[1]):  # Iterate over samples
            y_eval[i, k] = torch.sum(coef[i, :] * B[i, :, k])
