...
#--------------------------------------------------------face预处理 得到特征--------------------------------------------------------------------------
    ...
    def decode( #decoder输入：torchsize(1,33564,576) 长度33564待定，维度576固定
        ...
        condangle1 = self.incident_angle_linear1(in_angle).reshape(x.shape[1],96,-1) #torch.Size([10, 1, 26976])
        ...
        condfreq1 = self.emfreq_embed1(discretized_freq).reshape(x.shape[1],96,-1) #torch.Size([10, 1, 26976])
        ...
        # checksize(x)
        x = x + condangle2 #torch.Size([10, 96, 4050])
        x = x + condfreq2
        ...
        x = self.conv1_2(x)
        x = self.bn1_2(x)
        x = F.silu(x) #torch.Size([10, 48, 90, 180])
        x = x + condangle3
        x = x + condfreq3
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2] * x.shape[3])
        ...
        x = self.conv2_2(x)
        x = self.bn2_2(x)
        x = F.silu(x) #torch.Size([10, 24, 180, 360])
        x = x + condangle4
        x = x + condfreq4
        # print(x.shape, x.shape[0] * x.shape[1] * x.shape[2] * x.shape[3])
